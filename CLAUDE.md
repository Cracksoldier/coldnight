...
| `{% video url [caption] %}` | tag (inline) | Responsive 16:9 video embed. Supports YouTube (`youtube.com/watch?v=` and `youtu.be/`), Vimeo, and direct `.mp4`/`.webm`/`.ogv` files. Unrecognised URLs render as a plain link. Caption words (optional, space-separated) render as `<figcaption>` below the embed. Styles in `_components.scss` under `// ─── Video embed`. |
| `after_render:html` filter | filter | **Pass 1**: Injects `data-lang` on `<figure class="highlight <lang>">` for CSS language labels; also extracts `// filename:` (or `#`/`/* ... */` variants) as the first line of a code block, strips it from displayed code, and adds `data-filename` to the figure. **Pass 2**: Converts mermaid code blocks to `<div class="mermaid">` when `theme.mermaid.enabled`. **Pass 3**: Converts `<p><img></p>` to `<figure><img><figcaption>` when `theme.image_captions` is enabled. **Pass 4**: Adds `loading="lazy"` to all `<img>` tags not already carrying that attribute. |
...
**Filename label convention**: place `// filename: path/to/file.ext` (or `# filename:` for Python/shell, `/* filename: ... */` for CSS) as the **first line** of any fenced code block. The `after_render:html` Pass 1 strips the comment from the displayed code and sets `data-filename` on the `<figure>`. `copy-code.js` reads `data-filename` and prepends a `.code-filename-label` chip in the toolbar to the left of the language label (order: `[ src/main.js  js  📋 ]`). The filename chip uses `$text-primary` colour and a right border to separate it from the lang label. CSS in `_code.scss`.

`source/js/copy-code.js` injects a `.code-toolbar` flex div (filename label [optional] + language label + copy button) into the top-right of each `figure.highlight`. The copy handler targets `td.code` explicitly to exclude line numbers. The toolbar is always visible; the copy button highlights on hover. Clipboard writes use `navigator.clipboard` with an `execCommand('copy')` fallback for non-HTTPS contexts.
